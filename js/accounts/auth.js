// js/accounts/auth.js
// Custom seed phrase implementation for browser compatibility
...
const WORD_LIST = [
    ...
    "birth", "bishop", "bit", "bite", "bitter", "black", "blade", "blame", "blanket", "blast",
    ...
    "patient", "patron", "pattern", "pause", "pave", "payment", "peace", "peanut", "pear", "peasant",
    ...
    "rent", "reopen", "repair", "repeat", "replace", "report", "require", "rescue", "result", "return",
    "reveal", "review", "reward", "ride", "ridge", "rifle", "right", "rigid", "ring", "riot",
    ...
    "strike", "strong", "struggle", "student", "stuff", "stumble", "style", "subject", "submit", "sudden",
    ...
    "topple", "torch", "tornado", "tortoise", "toss", "trace", "track", "trade", "traffic", "tragic",
    ...
    "work", "worker", "world", "worry", "worth", "wrap", "wreck", "wrestle", "wrist", "write",
...
export class AuthManager {
    ...
    async init() {
        // Check if seed is stored locally
        const storedSeed = localStorage.getItem('user_seed');
        if (storedSeed) {
            this.seed = storedSeed;
            this.user = this.deriveUserFromSeed(storedSeed);
    ...
    deriveUserFromSeed(seed) {
        const hash = CryptoJS.SHA256(seed).toString();
        return {
            id: hash,
            email: null, // No email in seed auth
            name: 'Seed User'
        };
    }
    ...
    async signInWithSeed(seedPhrase) {
        try {
            if (!this.validateSeed(seedPhrase)) {
                throw new Error('Invalid seed phrase');
            }
            this.seed = seedPhrase;
            this.user = this.deriveUserFromSeed(seedPhrase);
            localStorage.setItem('user_seed', seedPhrase);
            this.updateUI(this.user);
            this.authListeners.forEach(cb => cb(this.user));
            console.log('Sign In successful with seed');
            return this.user;
        } catch (error) {
    ...
    async signUpWithSeed() {
        try {
            const seed = this.generateSeed();
            this.seed = seed;
            this.user = this.deriveUserFromSeed(seed);
            localStorage.setItem('user_seed', seed);
            this.updateUI(this.user);
            this.authListeners.forEach(cb => cb(this.user));
            console.log('Sign Up successful, seed generated');
            return { user: this.user, seed: seed };
        } catch (error) {
    ...
    async signOut() {
        try {
            this.user = null;
            this.seed = null;
            localStorage.removeItem('user_seed');
            this.updateUI(null);
    ...
    updateUI(user) {
        ...
        if (user) {
            ...
            if (accountUserEmail) accountUserEmail.textContent = `Seed Authenticated`;
            if (accountUserId) accountUserId.textContent = `User ID: ${user.id.substring(0, 8)}...`;
        } else {
...
document.addEventListener('DOMContentLoaded', () => {
    // Seed sign in
    const seedSigninBtn = document.getElementById('seed-signin-btn');
    if (seedSigninBtn) {
        seedSigninBtn.addEventListener('click', async () => {
            ...
            try {
                await authManager.signInWithSeed(seedPhrase);
    ...
    // Generate new seed
    const seedGenerateBtn = document.getElementById('seed-generate-btn');
    if (seedGenerateBtn) {
        seedGenerateBtn.addEventListener('click', () => {
            const seedAuthContainer = document.getElementById('seed-auth-container');
            const seedDisplayContainer = document.getElementById('seed-display-container');
            const generatedSeedEl = document.getElementById('generated-seed');

            const seed = authManager.generateSeed();
            generatedSeedEl.textContent = seed;

            seedAuthContainer.style.display = 'none';
            seedDisplayContainer.style.display = 'block';
        });
    }

    // Confirm seed saved
    const seedConfirmBtn = document.getElementById('seed-confirm-btn');
    if (seedConfirmBtn) {
        seedConfirmBtn.addEventListener('click', async () => {
            const generatedSeedEl = document.getElementById('generated-seed');
            const seed = generatedSeedEl.textContent;
            try {
                const result = await authManager.signUpWithSeed();
                alert(`Account created! Your seed phrase is: ${seed}\n\nKeep it safe - this is your only way to access your account.`);
            } catch (error) {
    ...
    const seedCancelBtn = document.getElementById('seed-cancel-btn');
    if (seedCancelBtn) {
        seedCancelBtn.addEventListener('click', () => {
            const seedAuthContainer = document.getElementById('seed-auth-container');
            const seedDisplayContainer = document.getElementById('seed-display-container');

            seedDisplayContainer.style.display = 'none';
            seedAuthContainer.style.display = 'block';
        });
    }

    // View seed phrase
    const viewSeedBtn = document.getElementById('account-view-seed-btn');
    if (viewSeedBtn) {
        viewSeedBtn.addEventListener('click', () => {
            const seed = authManager.getSeed();
            if (seed) {
    ...
    if (signoutBtn) {
        signoutBtn.addEventListener('click', async () => {
            if (confirm('Are you sure you want to sign out? Make sure you have your seed phrase saved.')) {
                try {
